...
// WBTC - Wrapped Bitcoin (8 decimals like Bitcoin)
const TOKEN_NAME: &str = "Wrapped Bitcoin";
const TOKEN_SYMBOL: &str = "WBTC";
...
pub extern "C" fn call() {
    // 1,000 WBTC initial supply
    let initial_supply = U256::from(1_000u64) * U256::exp10(TOKEN_DECIMALS as usize);
    ...
    let (contract_hash, _) = storage::new_contract(get_entry_points(), Some(named_keys), Some("wbtc_token_package".to_string()), Some("wbtc_token_access".to_string()));
    runtime::put_key("wbtc_token_contract", contract_hash.into());
